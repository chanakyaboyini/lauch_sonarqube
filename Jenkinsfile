pipeline {
  ...
  environment {
    ...
    // If using Jenkins AWS Credentials plugin:
    ...
    stage('Prepare AWS CLI & User Data') {
      steps {
        sh '''#!/bin/bash
          ...
          # Tune for Elasticsearch (required by SonarQube)
          sysctl -w vm.max_map_count=262144
          echo 'vm.max_map_count=262144' > /etc/sysctl.d/99-sonarqube.conf

          # Detect Amazon Linux 2 / 2023 vs Ubuntu basics (we use Amazon Linux via SSM below)
          ...
          # Run SonarQube (LTS Community). Default creds: admin/admin (will force password change).
      ...
      steps {
        withAWS(credentials: "${env.AWS_CRED_ID}", region: "${env.AWS_REGION}") {
          sh '''
            ...
            # Your public IP to restrict access (fallback to 0.0.0.0/0 if it fails)
            ...
            # Allow SSH (22) and SonarQube (9000)
  ...
  post {
    ...
    // Optional: automatic teardown after the job; comment out if you want to keep the server running
    // success {
    //   withAWS(credentials: "${env.AWS_CRED_ID}", region: "${env.AWS_REGION}") {
    //     sh '''
    //       set -euo pipefail
